...
import akka.testkit.{ImplicitSender, TestKit, EventFilter}
...
class SupervisionSpec extends TestKit(ActorSystem("SupervisionSpec"))
  ...
  "A supervisor" should {
    ...
    "resume its child in case of a minor fault" in {
      ...
      child ! "Akka is awesome because I'm learning to think in a whole new way"
      child ! Report
      expectMsg(3)
    }
    ...
    "terminates its child in case a major error" in {
      val supervisor = system.actorOf(Props[Supervisor])
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
